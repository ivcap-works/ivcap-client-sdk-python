...
api_dir = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "build/sdk_client/ivcap_client/api"
)
model_dir = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "build/sdk_client/ivcap_client/models"
)
...
def fix_job_status(line):
    ...
    line = line.replace(
        "request_content = File(payload=BytesIO(_request_content))",
        "request_content = _request_content",
    )

    line = line.replace(
        "result_content: Union[Unset, File]", "result_content: Union[Unset, Any]"
    )
    line = line.replace(
        "result_content = File(payload=BytesIO(_result_content))",
        "result_content = _result_content",
    )

    line = line.replace(
        "request_content: Union[Unset, FileJsonType] = UNSET",
        "request_content: Union[Unset, Any] = UNSET",
    )
    line = line.replace(
        "request_content = self.request_content.to_tuple()",
        "request_content = self.request_content",
    )

    line = line.replace(
        "result_content: Union[Unset, FileJsonType] = UNSET",
        "result_content: Union[Unset, Any] = UNSET",
    )
    line = line.replace(
        "result_content = self.result_content.to_tuple()",
